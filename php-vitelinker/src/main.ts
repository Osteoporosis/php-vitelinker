...
function parseCommandLineArgs() {
  ...
  if (!options.dist) {
    console.error(
      `Output directory is required. Usage: npx php-vitelinker ${argumentsNames} ${prefixFlags} ${distFlags}`
    );
    process.exit(1);
  }
...
function writePHPFile(
  ...
  const importTagAll = (entry.imports ?? [])
    .map((i) => `<link rel="modulepreload" href="${prefixPath}${i}" />`)
    .join("\n");
  const cssTagAll = (entry.css ?? [])
    .map((i) => `<link rel="stylesheet" href="${prefixPath}${i}" />`)
    .join("\n");
...
function writePHPFiles(prefixPath: string, distPath: string) {
  ...
  const entries = Object.entries(jsonData).filter(
    ([, entry]: [string, Entry]) =>
      Object.prototype.hasOwnProperty.call(entry, "isEntry") &&
      entry.isEntry === true
  );
  entries.map(([src, entry]) => {
    writePHPFile(src, entry, prefixPath, distPath);
  });
}
